  ...
  <div class="space-y-10">
    ...
    <div class="flex items-end justify-between">
      <div>
        ...
        <h1 class="font-display text-3xl md:text-4xl font-semibold" style="color: var(--text)">
          即時金價
        </h1>
      </div>
    ...
    <!-- Main price display -->
    <div v-if="goldStore.current" class="grid grid-cols-1 sm:grid-cols-2 gap-px" style="background: var(--border); border: 1px solid var(--border); border-radius: 16px; overflow: hidden;">
      <!-- Buy price -->
      <div class="p-6 md:p-8" style="background: var(--surface)">
        <p class="stat-label mb-3">存摺買入</p>
        <p class="price-value text-4xl md:text-5xl font-medium" style="color: var(--text)">
        ...
        <div class="mt-4 flex items-center gap-2">
          <span
            ...
            :style="goldStore.current.change >= 0 ? 'color: var(--up)' : 'color: var(--down)'"
          >
            {{ goldStore.current.change >= 0 ? '+' : '' }}{{ goldStore.current.change.toFixed(0) }}
          </span>
          <span
      ...
      <div class="p-6 md:p-8" style="background: var(--surface)">
        ...
        <div class="mt-4">
          <span class="stat-label">
            買賣價差 <span class="price-value" style="color: var(--text-2)">{{ (goldStore.current.todaySell - goldStore.current.todayBuy).toLocaleString() }}</span> 元
          </span>
        </div>
      </div>
    </div>
    ...
    <!-- Loading skeleton -->
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-px" style="background: var(--border); border: 1px solid var(--border); border-radius: 16px; overflow: hidden;">
      <div v-for="i in 2" :key="i" class="p-8 h-44 animate-pulse" style="background: var(--surface-2)" />
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div class="lg:col-span-2">
        <div class="flex items-center justify-between mb-5">
          <p class="text-sm font-medium" style="color: var(--text)">近30日走勢</p>
          <NuxtLink to="/chart" class="stat-label transition-colors" style="color: var(--gold)">查看完整圖表 →</NuxtLink>
        </div>
        <ClientOnly>
      ...
      <div>
        <div class="flex items-center justify-between mb-5">
          ...
          <NuxtLink to="/news" class="stat-label" style="color: var(--gold)">更多 →</NuxtLink>
        </div>
        <div v-if="newsStore.loading" class="space-y-4">
          <div v-for="i in 3" :key="i" class="h-12 animate-pulse rounded" style="background: var(--surface-2)" />
        </div>
        <div v-else class="space-y-5">
          <a
            ...
            class="block group"
          >
            <p class="text-sm leading-snug line-clamp-2 transition-colors" style="color: var(--text-2)">
              <span class="group-hover:underline decoration-[var(--gold)] underline-offset-2">{{ item.title }}</span>
            </p>
            <p class="stat-label mt-1">{{ item.source }} · {{ formatDate(item.publishedAt, 'MM/DD HH:mm') }}</p>
          </a>
        </div>
      </div>
    </div>
...
const miniChartOption = computed<EChartsOption>(() => {
  ...
  return {
    ...
    series: [{
      ...
      areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(196,151,63,0.15)' }, { offset: 1, color: 'rgba(196,151,63,0)' }] } },
      smooth: true,
      showSymbol: false,
    }],
    tooltip: {
