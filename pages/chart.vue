<template>
  <div class="space-y-8">
    ...
    <div class="flex items-end justify-between">
      <div>
        <p class="stat-label mb-1">黃金存摺</p>
        <h1 class="font-display text-3xl font-semibold" style="color: var(--text)">走勢圖</h1>
      </div>
      <!-- Range selector -->
      <div class="flex items-center gap-1 p-1 rounded-lg" style="background: var(--surface-2); border: 1px solid var(--border)">
        <button
          v-for="r in ranges"
          :key="r.value"
          class="px-3 py-1.5 rounded-md text-xs font-medium transition-all"
          :style="goldStore.selectedRange === r.value
    ...
    <div class="rounded-2xl overflow-hidden" style="border: 1px solid var(--border); background: var(--surface)">
      ...
      <div class="p-4">
        <div v-if="goldStore.loadingHistory" class="h-[360px] animate-pulse rounded-lg" style="background: var(--surface-2)" />
        <ClientOnly v-else>
    ...
    <div v-if="goldStore.current" class="grid grid-cols-2 md:grid-cols-4 gap-6">
      <div v-for="s in priceStats" :key="s.label">
        ...
        <p class="price-value text-lg font-medium" :style="`color: ${s.color}`">{{ s.value }}</p>
      </div>
    </div>
...
const rangeChangeMap: Record<string, { label: string; key: 'weekChange' | 'monthChange' | 'threeMonthChange' | 'yearChange' }> = {
...
const priceStats = computed(() => {
  ...
  const rv = c[rc.key]
  return [
    { label: '存摺買入價', value: `${c.todayBuy.toLocaleString()} 元/克`, color: 'var(--text)' },
    { label: '存摺賣出價', value: `${c.todaySell.toLocaleString()} 元/克`, color: 'var(--text)' },
    { label: '今日漲跌', value: `${sign(c.change)} (${formatPercent(c.changePercent)})`, color: col(c.change) },
...
const chartOption = computed<EChartsOption>(() => {
  ...
  return {
    ...
    xAxis: {
      ...
      axisLabel: { color: labelColor.value, fontSize: 10, rotate: data.length > 90 ? 30 : 0 },
    ...
    yAxis: {
      ...
      axisLabel: { color: labelColor.value, fontSize: 10, formatter: (v: number) => v.toLocaleString() },
